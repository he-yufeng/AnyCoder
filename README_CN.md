...
- **搜索替换编辑** - 精确修改文件，唯一性校验，diff 输出
- **危险命令拦截** - 拦截 `rm -rf /`、fork bomb、`curl | bash` 等
- **并行工具执行** - 多个独立工具调用同时跑
- **会话持久化** - `/save` 保存，`--resume` 恢复
- **`.env` 支持** - 项目根目录放个 `.env` 就行
- **~1,450 行 Python** - 可以看完，可以改，可以拿去造自己的
...
AnyCoder 有 7 个内置工具，大模型会自动调用：

| 工具 | 功能 |
|------|------|
| `bash` | 执行 Shell 命令，危险命令拦截，cd 追踪 |
| `read_file` | 读文件，带行号，支持大文件分段读取 |
| `write_file` | 创建新文件或覆盖已有文件 |
| `edit_file` | 搜索替换编辑，唯一性校验，diff 输出 |
| `glob` | 按模式搜索文件（`**/*.py`、`src/**/*.ts`） |
| `grep` | 正则搜索文件内容 |
| `run_tests` | 自动探测并运行项目测试（pytest、npm/pnpm/yarn），失败输出原样带回 |
...
| `/diff` | 查看本次会话修改的文件 |
| `/undo` | 撤销最近一次文件改动 |
| `/plan` | 开关 plan 模式（变更类工具调用执行前先审批） |
| `/compact` | 手动压缩上下文 |
| `/save [名称]` | 保存会话（名称会先安全化，再作为文件名） |
| `/sessions` | 列出已保存的会话 |
| `/clear` | 清空对话记录 |
| `/help` | 查看帮助 |
| `/quit` | 退出 |
...
一共 ~1,600 行，全看完也就一两个小时的事：

```
anycoder/
├── cli.py            REPL + 命令                   258 行
├── llm.py            litellm 流式封装              184 行
├── agent.py          Agent 循环 + 并行执行         179 行
├── context.py        两阶段上下文压缩               92 行
├── config.py         环境变量 + .env + 别名         86 行
├── session.py        会话保存/恢复                   83 行
├── prompts/system.py 系统提示词                      51 行
└── tools/
    ├── bash.py       Shell + 安全 + cd 追踪        130 行
    ├── edit_file.py  搜索替换 + diff 输出           98 行
    ├── grep_tool.py  正则搜索 + 跳过二进制         111 行
    ├── run_tests.py  探测并运行项目测试            113 行
    ├── read_file.py  文件读取 + 二进制检测          70 行
...
| 流式输出 | 支持 | 支持 | 支持 | **支持** |
| 会话持久化 | 支持 | 不支持 | 支持 | **支持** |
| 代码量 | 51 万行 | 10 万+ | 5 万+ | **~1,600 行** |
| 适合 | 直接用 | 直接用 | 直接用 | **用 + 读源码** |

## 后续规划

AnyCoder 的卖点就是整个 agent 装得进脑子里，所以规划是关于「能干什么」而不是「堆多大」。任何要加的东西都得让源码继续保持在 ~1500 行内可读。

- **MCP 客户端支持**：让 AnyCoder 把 Model Context Protocol 服务器当工具来用，同一个 agent 就能驱动任意 MCP 服务器暴露的能力。
- **先计划后执行模式**：先把计划和打算改动的文件列出来，确认一次再执行，给想在落盘前留个检查点的人用。
- **可插拔编辑策略**：默认是搜索替换；再加一个 diff/patch 策略，能更干净地处理大文件和多处改动。

如果一个功能不把源码读起来变难就加不进来，那它就不该进来。这就是 AnyCoder 的全部主张。
